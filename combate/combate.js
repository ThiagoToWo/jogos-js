...
function lutar(lutador1, lutador2, relator) {
	...
	if (teste_foco <= atacante.foco) {
		...
		_relatar(atacante.nome, 'efetuou golpe', ataque.toFixed(), '/', atacante.ataque);
	} else {
	...
	if (teste_magia <= atacante.estilo && teste_habilidade <= atacante.habilidade) {
		...
		_relatar(atacante.nome, 'usou magia', magia.toFixed(), '/', atacante.magia);
	} else {
	...
	if ((ataque || magia) && teste_reflexo <= defensor.reflexo) {
		...
		_relatar(defensor.nome, 'conseguiu defender', defesa.toFixed(), '/', defensor.defesa);
	} else {
